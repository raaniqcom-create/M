...
export function AudienceBanner({
  ...
  if (ticker) {
    // بلا مبالغة — «مبالغةٌ عاليةٌ جدّاً بالحديث»، قال صاحبُ المنصّة عن
    // الصيغة الأولى. رقمٌ ومكانٌ وفعلٌ واحد.
    const items = !updatedToday
      ? [
          `${n} شخص في ${station.city} ينتظرون خبرك اليوم`,
          'اضبط المنتجات ثمّ اضغط الزرَّ الأخضر — يصلهم إشعارٌ واحد',
        ]
      : [
          `${n} شخص في ${station.city} يصلهم ما تنشره الآن`,
          ...(f > 0 ? [`${f} منهم يتابعون محطتك`] : []),
        ];

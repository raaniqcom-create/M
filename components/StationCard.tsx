...
export function StationCard({ station }: { station: StationWithStatus }) {
  const availability = new Map(station.products.map((p) => [p.product, p.is_available]));
    ...
    <article className="card p-4">
      ...
      <ul className="mt-3 flex flex-wrap gap-1.5">
        {PRODUCT_ORDER.map((product) => {
          const available = availability.get(product) ?? false;
          return (
            <li
              key={product}
              className={`rounded-full px-2.5 py-1 text-xs font-semibold ${
                available ? 'bg-brand-100 text-brand' : 'bg-slate-100 text-slate-400 line-through'
              }`}
            >
      ...
      <p className="sr-only">
        المتوفر:{' '}
        {PRODUCT_ORDER.filter((p) => availability.get(p))

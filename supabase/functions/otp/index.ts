...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'direct') {
      ...
      // ── والإيقافُ لا يُرفع عنه الرمز ────────────────────────────────────
      //
      // **والفرقُ بين البابين ليس ذوقاً.** من اشترك بلا إثباتٍ يصله ما لم
      // يطلبه، ويُوقفه بنفسه من هذه الشاشة. ومن أُوقف بلا إثباتٍ **يسكت عنه
      // الخبرُ ولا يدري** — فلا يعرف أنّ شيئاً يُنتظر ولا يأتي.
      //
      // ومفتاحُ anon منشورٌ في حزمة الموقع، وحدُّ الدقيقة والعشرِ في الساعة
      // قائمٌ في `send` وحدَه — فبابٌ بلا رمزٍ هنا يُسكت إشعاراتِ أيّ مشترك،
      // واحداً بعد واحد، بلا حدّ. وهم أكثرُ من واحدٍ وعشرين ألفاً.
      //
      // وصاحبُ المنصّة قال «ارفع التسجيل عن otp» — والتسجيلُ اشتراكٌ لا إيقاف.
      if (purpose === 'unsubscribe') {
        return json({ error: 'إيقافُ الرسائل يحتاج رمزاً' }, 400);
      }

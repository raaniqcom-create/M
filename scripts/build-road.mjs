 ...
 *
 *  2 و3 حُذفتا — إعادتان لما يقوله 4. و**11 مثلُهما**: قِيست أطيافُ المقاطع
 *  السبعة الجديدة وقُورنت كلُّ اثنين، فكانت مسافةُ 11 و12 أربعةً وخمسةَ عشر
 *  من مئة، والوسيطُ أحدَ عشرَ وثلاثةَ عشر، وأقربُ ما بعدهما تسعةً وسبعةً
 *  وثلاثين. أي أنهما تلاوتان لنصٍّ واحد — «ولكلِّ طريقٍ امتدادُه الأطول».
 *
...
//
//  **وعاد إلى آخره.** كان ينتهي عند الرابعة والستّين، إذ لم يكن للفقرات
...
/** نهايةُ آخرِ كلمةٍ مسجَّلة، وبعدها الذيل */
const voiceEnd = +(t - GAP + TAIL).toFixed(2);
/** والمشاهدُ التي لا تعليقَ لها تُضاف بزمنها كما هو */
const SILENT = OLD.at(-1) - OLD[OLD.indexOf(LAST_VOICED) + 1];
const total = +(voiceEnd + SILENT).toFixed(2);
 ...
 *    8     «ثمّ يبدأُ أطولُ امتداد»   سمعه عند 89.8، والشريحةُ كانت عند 80.0
 *
 *  وبهذه الثمانِ صار لكلِّ مقطعٍ أوّلُ فقرةٍ مؤكَّدٌ سماعاً — وأكّد المالك
 *  أن العاشر يقول «وثلاثةُ منافذ»، فلم يبقَ مجهولاً إلا موضعُ الحدّ بين
 *  «ثمّ يبدأ» و«وبعدها» داخلَ المقطع التاسع. */
const ANCHOR = new Map([[0, 0], [1, 1], [2, 2], [3, 4], [4, 6], [6, 21], [7, 28], [8, 32]]);

/** ومِرساةٌ بالمقطع لا بوحدة النطق: `فقرة -> رقمُ المقطع الذي تبدأ بأوّله`.
 *
 *  **وهي أمتنُ من رقمٍ مكتوبٍ بيد.** رقمُ وحدةِ النطق يتغيّر بتغيّر التقطيع
 *  — عتبةٌ أو سكتةٌ أو مقطعٌ يُضاف — فتصير المِرساةُ تشير إلى غير موضعها
 *  صامتةً. ورقمُ المقطع لا يتغيّر: هو اسمُ ملفٍّ على القرص.
 *
 *  والمقاطعُ الستّةُ الأخيرة كلٌّ منها فقرةٌ واحدة. قِيست بالوتيرة المؤكَّدة
 *  فوسعت ما بين مئةٍ وعشرةٍ ومئتين وستّين حرفاً، وعُدّت وحداتُ نطقها فوافقت
 *  وحداتِ ترقيم فقراتها — أربعٌ لثلاثِ مسافاتٍ ونقطتين، وثلاثٌ للخاتمة، واثنتا
...
for (const [para, clip] of ANCHOR_CLIP) {
  ...
  if (at < 0) {
    console.log(`مِرساةٌ إلى مقطعٍ ليس في CLIPS: ${clip}.mp3`);
    process.exit(1);
  }
  ANCHOR.set(para, at);
}
...
const NEW = OLD.map((o, i) =>
  i === 0 ? 0
  : o <= LAST_VOICED ? sceneAt.get(o)
  : +(voiceEnd + (o - OLD[OLD.indexOf(LAST_VOICED) + 1])).toFixed(2)
);

 ...
 *  ── وككلماتٍ لا كنصّ ────────────────────────────────────────────────────
 *
 *  **كان يأكل الأسماءَ من داخلها.** «غدا» تقع حرفيّاً داخل «بغداد»، فكلُّ اسمٍ
 *  فيه هذه الكلمةُ يُمزَّق: «البغدادية» صارت «الب دية»، و«البغدادي» صارت
 *  «الب دي». وقِيس مرّتين على جدولٍ حقيقيّ — السطر ٢٥ من جدول ٢٠٢٦-٠٩-١١
 *  حُفظ «الب دية الكرمة»، ووقعت قبلها في «الجبل الب دي».
 *
 *  فالحدُّ حرفٌ عربيٌّ قبلَه أو بعدَه: `` في جافاسكربت لا تقع بين حرفين
 *  عربيّين (`lib/nearbyFuel.ts:84`)، فتُكتب النظرةُ صريحةً.
 *
 *  والنظرةُ الأماميّةُ **قبل** `\s*` لا بعدَها: لو وقعت بعد المسافات لَفحصت
 *  أوّلَ حرفٍ من اسم المحطة — وهو عربيٌّ دائماً — فلا تُسقَط «غدا» أبداً. */
// والمدى `[ء-ي]` هو `\u0621-\u064A` نفسُه، مكتوباً كما يُقرأ.
// وتعبيرٌ حرفيٌّ لا `new RegExp` بقالبٍ نصّيّ: القالبُ يبتلع \s فتصير `s`،
// فيُطابَق حرفُ السين بدل الفراغ — ويسقط الشرطُ صامتاً.
const PREAMBLE =
...
function matchLoose(
  ...
  if (!hit) {
    ...
    return {
      ...
      // ── ومحطةٌ مسجّلةٌ تعرف مدينتَها، فلا تُترك «منطقةً لم أعرفها» ────────
      //
      // **وقع مقيساً: ستَّ عشرةَ محطةً من تسعٍ وأربعين** — «ذراع دجلة»
      // و«ساسكو» و«عطارد» وغيرُها — تُربط بمعرّفها الصحيح ويخرج سطرُها بلا
      // منطقة. وهذا الفرعُ يقع حين لا يجد المسحُ مرشَّحاً: فتُقرأ المدينةُ من
      // نصّ السطر وحدَه، ولا تُسأل المحطةُ التي رُبطت بها لتوّها.
      //
      // وليس تجميلاً في المعاينة: `schedCities` تجمع مدنَ الأسطر، فمدينةٌ
      // فارغةٌ تعني **أنّ أهلَ تلك المدينة لا يصلهم إشعارُ الجدول**. وشبكةُ
      // الصباح تقرأ العمودَ نفسَه، ولوحةُ الجدول تجمع به.
      //
      // ومدينةُ السطر تسبق: من كتبها أراد ما كتب.
      city: direct?.city ?? cityInText(raw) ?? null,
  ...
  return {
    ...
    city: (() => {
      ...
      // ── ومحطةٌ مسجّلةٌ تعرف مدينتَها، فلا تُترك «منطقةً لم أعرفها» ────────
      //
      // **وقع مقيساً:** «ذراع دجلة» مسجّلةٌ ومعتمدةٌ ومدينتُها «الكرمة» في
      // القاعدة، وكان سطرُها يخرج بلا منطقة — لأنّ المدينةَ كانت تُؤخذ من
      // مرشَّح المسح (`c`) وحدَه، ومن اسم السطر، ولا تُسأل المحطةُ المربوطة.
      //
      // وليس تجميلاً في المعاينة: `schedCities` تجمع مدنَ الأسطر، فمدينةٌ
      // فارغةٌ تعني **أنّ أهلَ تلك المدينة لا يصلهم إشعارُ الجدول**. وشبكةُ
      // الصباح تقرأ العمودَ نفسَه.
      //
      // وتأتي بعد مدينة السطر: من كتب المدينةَ في السطر أراد ما كتب.
      //
      // **وتسبق مدينةَ المسح.** المسحُ سجلٌّ من طرفٍ ثالث، ومدينةُ المحطة
      // المسجّلة كتبها صاحبُها وهي المعروضةُ على صفحتها. فلولا هذا الترتيب
      // لَادّعى سطرُ الجدول مدينةً تخالف صفحةَ المحطة نفسِها — وقِيس: ستُّ
      // محطاتٍ من تسعٍ وأربعين، منها «دار السلام» يقول صاحبُها الفلوجة
      // ويقول المسحُ الكرمة.
      return near?.city ?? c ?? lc ?? null;
    })(),
